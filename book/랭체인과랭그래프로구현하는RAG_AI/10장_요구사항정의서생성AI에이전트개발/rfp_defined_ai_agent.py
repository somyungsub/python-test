...
# 페르소나 리스트를 표현하는 데이터 모델
class Personas(BaseModel):
    personas: list[Persona] = Field(..., description="페르소나 리스트")
...
# 인터뷰 결과 리스트를 표현하는 데이터 모델
class InterviewResults(BaseModel):
    interviews: list[Interview] = Field(..., description="인터뷰 결과 리스트")
...
# 요구사항 정의 생성 AI 에이전트의 스테이트
class InterviewState(BaseModel):
    user_request: str = Field(..., description="사용자 요청")
    personas: Annotated[list[Persona], operator.add] = Field(default_factory=list, description="생성된 페르소나 리스트")
    interviews: Annotated[list[Interview], operator.add] = Field(default_factory=list, description="실시된 인터뷰 리스트")
    requirements_doc: str = Field(default="", description="생성된 요구사항 정의")
    iteration: int = Field(default=0, description="페르소나 생성과 인터뷰 반복 횟수")
    is_information_sufficient: bool = Field(default=False, description="정보가 충분한지 여부")
    evaluation_reason: str = Field(default="", description="정보 충분성 판단 이유")
...
class PersonaGenerator:
    ...
    def run(self, user_request: str) -> Personas:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ...
            ("human", f"다음 사용자 요청에 관한 인터뷰용으로 {self.k}명의 다양한 페르소나를 생성해주세요\n\n "
                      "사용자 요청: {user_request}\n\n"
                      "각 페르소나에는 이름과 간단한 배경을 포함해 주세요. 나이, 성별, 직업, 기술적 전문지식에서 다양성을 확보해 주세요.",)
        ])
...
class InterviewConductor:
    ...
    def run(self, user_request: str, personas: list[Persona]) -> InterviewResults:
        # 질문 생성
        ...
        # 인터뷰 결과 반환
        return InterviewResults(interviews=interviews)

    def _generate_questions(self, user_request: str, personas: list[Persona]) -> list[str]:
        # 질문 생성을 위한 프롬프트 정의
        question_prompt = ChatPromptTemplate.from_messages([
            ("system", "당신은 사용자 요구사항에 기반하여 적절한 질문을 생성하는 전문가 입니다."),
            ("human", f"다음 페르소나와 관련된 사용자 요청에 대해 질문 하나를 생성해 주세요.\n\n"
                      "사용자 요청: {user_request}\n"
                      "페르소나: {persona_name} - {persona_background}\n\n"
                      "질문은 구체적이고, 이 페르소나의 관점에서 중요한 정보를 끌어낼 수 있도록 설계해 주세요.")
        ])
    ...
    def _create_interviews(self, personas: list[Persona], questions: list[str], answers: list[str]) -> list[Interview]:
        ...
        return [
            Interview(persona=persona, question=question, answer=answer) for persona, question, answer in zip(personas, questions, answers)
        ]
...
class InformationEvaluator:
    ...
    def run(self, user_request:str, interviews: list[Interview]) -> EvaluationResult:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ...
            ("human", "다음 사용자 요청과 인터뷰 결과를 바탕으로, 포괄적인 요구사항 문서를 작성하기에 충분한 정보가 모였는지 판단해 주세요.\n\n"
                      "사용자 요청: {user_request}\n\n"
                      "인터뷰 결과: \n{interview_results}")
        ])
...
class RequirementsDocumentGenerator:
    ...
    def run (self, user_requests: str, interviews: list[Interview]) -> str:
        # 프롬프트 정의
        prompt = ChatPromptTemplate.from_messages([
            ("system", "당신은 수집한 정보를 바탕으로 요구사항 문서를 작성하는 전문가입니다."),
            ("human", "다음 사용자 요청과 여러 페르소나의 인터뷰 결과를 바탕으로, 요구사항 문서를 작성해 주세요.\n\n"
                      "사용자 요청: {user_requests}\n\n"
                      "인터뷰 결과: \n{interview_results}\n"
...
class DocumentationAgent:
    def __init__(self, llm: ChatOpenAI, k: Optional[int] = None):
        ...
        self.requirements_document_generator = RequirementsDocumentGenerator(llm=llm)
    ...
    def _conduct_interviews(self, state: InterviewState) -> dict[str, Any]:
        # 인터뷰 실시
        new_interviews: InterviewResults = self.interview_conductor.run(state.user_request, state.personas[-5:])
    ...
    def _generate_requirements(self, state: InterviewState) -> dict[str, Any]:
        # 요구사항 정의서 생성
        requirements_doc: str = self.requirements_document_generator.run(state.user_request, state.interviews)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="만들고 싶은 애플리케이션에 대해 기술해 주세요"
    )
    ...
    # ChatOpenAI 모델 초기화
    llm = create_model(model="gpt-4o", temperature=0.0)
